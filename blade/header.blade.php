...
function renderHeader() {
    const socials = [{
            href: 'https://x.com/MertDoganay61',
            icon: 'fab fa-twitter',
            title: 'Twitter'
        },
        {
            href: 'https://github.com/mert6148',
            icon: 'fab fa-github',
            title: 'GitHub'
        },
        {
            href: 'mailto:mertdoganay437@gmail.comA',
    ...
    const navLinks = [{
        ...
        {
            href: 'mailto:info@merthtmlcss.com',
            label: 'İletişim'
        }
    ];
    let navHtml = navLinks.map(l => `<a href="${l.href}" class="nav-link">${l.label}</a>`).join('');
    let socialHtml = socials.map(s =>
        `<a href="${s.href}" target="_blank" title="${s.title}"><i class="${s.icon}"></i></a>`).join('');
